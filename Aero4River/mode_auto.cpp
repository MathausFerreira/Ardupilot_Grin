...
void ModeAuto::wp_run(){
    ...
    if (!copter.failsafe.radio) {
        ...
        target_yaw_rate = channel_yaw->norm_input();  //get_pilot_desired_yaw_rate(channel_yaw->get_control_in());
    ...
    if (auto_yaw.mode() == AUTO_YAW_HOLD) {
        ...
        attitude_control->input_fx_fy_rate_yaw(wp_nav->get_roll(), wp_nav->get_pitch(), target_yaw_rate);
    } else {
...
void ModeAuto::circle_run()
{
    ...
    if (auto_yaw.mode() == AUTO_YAW_HOLD) {
        // roll & pitch from waypoint controller, yaw rate from pilot
        attitude_control->input_fx_fy_rate_yaw(copter.circle_nav->get_roll(), copter.circle_nav->get_pitch(), target_yaw_rate);
    } else {
        // roll, pitch from waypoint controller, yaw heading from auto_heading()
